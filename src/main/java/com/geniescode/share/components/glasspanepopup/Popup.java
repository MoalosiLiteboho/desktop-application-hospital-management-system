...
public class Popup extends JComponent {
    ...
    private void initAnimator() {
        animator = new Animator(option.duration(), new TimingTargetAdapter() {
            ...
            public void timingEvent(float fraction) {
                if (show) animate = fraction;
                else animate = 1f - fraction;
    ...
    protected void paintComponent(Graphics graphics) {
        ...
        graphics2D.setComposite(AlphaComposite.SrcOver.derive(animate * option.opacity()));
        graphics2D.setColor(option.background());
        graphics2D.fill(new Rectangle2D.Double(0, 0, getWidth(), getHeight()));
        graphics2D.setComposite(AlphaComposite.SrcOver.derive(animate));
